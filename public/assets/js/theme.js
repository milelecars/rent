...
if (mainSlider.length) {
    mainSlider.owlCarousel({
        ...
        autoplaySpeed: 2000, // Adjust the speed here (value is in milliseconds)
        responsive: {
            0: { items: 1 },
            479: { items: 1 },
            768: { items: 1 },
    ...
    // Autoplay only for the background image (slide3) with zoom effect
    setInterval(function () {
        mainSlider.trigger('next.owl.carousel');
    }, 5000);  // Change background image every 5000 milliseconds (adjust as needed)

    // Apply zoom effect on background image change
    mainSlider.on('changed.owl.carousel', function (event) {
        var currentSlide = event.item.index;
        if (currentSlide === 2) { // Assuming slide3 is at index 2
            $('.main-slider .item').css('transform', 'scale(1.1)'); // Adjust the scale factor as needed
        } else {
            $('.main-slider .item').css('transform', 'scale(1)');
        }
    });
}
